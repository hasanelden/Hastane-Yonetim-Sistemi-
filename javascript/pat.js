...
$(document).ready(function() {
    ...
    $(".HastaEkle").click(function() {
        ...
        if (lst == "" || frst == "" || inno == "" || phone.length !== 14 || adress == "") {
    ...
    $(document).on('click', '.delete-btn', function() {
        ...
        swal({
                title: "Emin misin?",
                text: " Bir kere silindiğinde, Bu Hasta kaydını kurtaramazsınız!",
                icon: "warning",
                buttons: true,
                dangerMode: true,
                buttons: ["İPTAL", "SİL"]
            ...
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    swal("Hasta Kaydı Başarıyla silindi!", {
                        icon: "success",
                        buttons: true,
                        buttons: "KAPAT"
                    });
...
$(document).on('click', '.update-btn', function() {
    ...
    document.getElementById('inputIsim').value = edit[0].row[0].textContent;
    document.getElementById('inputSoyIsim').value = edit[0].row[1].textContent;
    document.getElementById('inputSgrto').value = edit[0].row[2].textContent;
    document.getElementById('inputTelefon').value = edit[0].row[3].textContent;
    document.getElementById('inputAdres').value = edit[0].row[4].textContent;

    $("#update").click(function() {


        edit[0].row[0].textContent = document.getElementById('inputIsim').value;
        edit[0].row[1].textContent = document.getElementById('inputSoyIsim').value;
        edit[0].row[2].textContent = document.getElementById('inputSgrto').value;
        edit[0].row[3].textContent = document.getElementById('inputTelefon').value;
        edit[0].row[4].textContent = document.getElementById('inputAdres').value;
        ...
        kayitliBilgiler.forEach(function(kisi) {
            ...
            kisi[2] = document.getElementById('inputSgrto').value;
        ...
        localStorage.setItem("hastaBilgileri", JSON.stringify(guncellenenBilgiler));

        if (document.getElementById('inputIsim').value == "" || document.getElementById('inputSoyIsim').value == "" || document.getElementById('inputSgrto').value == "" || document.getElementById('inputTelefon').value.length !== 14 || document.getElementById('inputAdres').value == "") {
...
function kisiSilLocaStorage(silinecekKisi) {
    ...
    localStorage.setItem("hastaBilgileri", JSON.stringify(kayitliBilgiler));
}
